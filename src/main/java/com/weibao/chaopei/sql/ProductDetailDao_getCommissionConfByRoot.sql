select pd.product_plan productPlan, rel.depart_id departid, rel.product_detail_id productPlanId, conf.id, conf.period, conf.rate 
...
join product_detail pd on rel.product_detail_id=pd.id
